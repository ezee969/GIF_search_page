class MainPage {
  ...
  async gifGenerator(searchTerm) {
    try {
      const response = await fetch(`https://api.giphy.com/v1/gifs/translate?api_key=bxej8jvrToGlsgtSGGfWF4RDDp8eqW11&s=${searchTerm}`, { mode: 'cors' });
      const data = await response.json();
      this.errorMesagge.style.display = 'none';
      this.gifDomRemove();
      this.img = document.createElement('img');
      this.img.src = data.data.images.original.url;
